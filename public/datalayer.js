...
method.getDatabaseList = function(){  
  var response = null, result = null;
     MongoClient.connect(url, function(err, db) {
         if (err) throw err;
         db.db('botdb').admin().listDatabases().then(function(result){
...
method.getCollectionList = function(databaseName){
  console.log('fetching collections for : databaseName:', databaseName);
    var response = null;
    MongoClient.connect(url, function(err, db) {
        if (err) console.log('error while fetching:', err);
...
method.getAccuracy = function(){  
  var response = null;
  console.log("call to get accuracy in datalayer");
  // var databases = JSON.parse(this.getDatabaseList());
